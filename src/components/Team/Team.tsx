  ...
  return (
    <div className="font-poppins pb-10 md:px-12 px-5 py-10 xl:px-40 md:py-20 lg:max-xl:max-w-6xl mx-auto">
      ...
      <div className="flex max-sm:flex-col gap-5 lg:gap-10 sm:px-16 justify-center items-center">
            ...
            <img src='./Tokunbo.png' className="w-[18.5rem] sm:w-[27.875rem]" />
            </div>
            
            <div className="text-head-black mt-6">
              {teamMember.map((data, index) => (
              <div key={index} className="flex items-center gap-10 xl:gap-20 border-b-2 mb-3 x:mb-5">
                <h1 className="w-20 lg:w-28 text-center text-sm lg:text-base xl:text-lg font-semibold mb-3">{data.name}</h1>
                <h4 className="mb-3 w-32 lg:w-40 xl:w-48 text-xs lg:text-sm xl:text-base text-center">{data.position}</h4>
